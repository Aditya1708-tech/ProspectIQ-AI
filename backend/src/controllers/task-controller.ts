...
export class TaskController {
  ...
  async list(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      const isManager = user.roles.includes('ADMIN') || user.roles.includes('BRANCH_MANAGER') || user.roles.includes('REGIONAL_MANAGER');
      if (!isManager) {
  ...
  async update(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      if (actualDuration !== undefined) updateData.actualDuration = parseInt(actualDuration, 10);
  ...
  async workload(req: AuthenticatedRequest, res: Response) {
    const user = req.user;
    if (!user) {
      return res.status(401).json({
        success: false,
    ...
    try {
      ...
      const rmWorkloadMap: Record<string, { rmName: string; pending: number; completed: number; overdue: number; total: number }> = {};
      tasks.forEach(t => {
