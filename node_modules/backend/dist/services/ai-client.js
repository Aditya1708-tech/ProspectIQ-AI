export class AIClient {
    ...
    async analyzeProfile(profile, requestId) {
        try {
    ...
    async simulateProfile(profile, scenario, requestId) {
        try {
    ...
    async analyzePortfolio(profiles) {
        try {
            const payload = {
    ...
    generateLocalFallback(profile) {
        ...
        const finalPriorityScore = Math.round((oppScore * 0.3) +
            (growthScore * 0.25) +
        ...
        const summaryText = (`The customer ${profile.customer.name} is classified in the ${profile.customer.segment} banking segment under the professional persona of '${personaName}', managed under branch reference ${profile.customer.branchCode}. ` +
    ...
    generateLocalPortfolioFallback(profiles) {
        ...
        for (const p of profiles) {
            if (p.customer.segment === 'MSME')
                msme++;
            else
                retail++;
            if (p.customer.status === 'ACTIVE')
                active++;
            else if (p.customer.status === 'DORMANT')
                dormant++;
            else
                prospects++;
        }
        ...
        const executiveSummary = (`The branch portfolio command center currently monitors ${total} customer relationship profiles, consisting of ` +
        ...
        const topOpportunities = profiles.slice(0, 10).map(p => ({
        ...
        const riskIntelligence = profiles.filter(p => !p.customer.email || p.customer.status === 'DORMANT').map(p => ({
    ...
    async getAdminDashboard(requestId) {
        try {
