...
export class ImportService {
    ...
    async importFromBuffer(buffer, mimeType, columnMapping = {}, rmId) {
        let rawRecords = [];
        const errors = [];
        let total = 0;
        let imported = 0;
    ...
    normalizeKey(key) {
        ...
        if (norm === 'customername' || norm === 'fullname' || norm === 'name')
            return 'name';
        if (norm === 'customeremail' || norm === 'emailaddress' || norm === 'email')
            return 'email';
        if (norm === 'customerphone' || norm === 'phonenumber' || norm === 'phone' || norm === 'mobile')
            return 'phone';
        if (norm === 'occupation' || norm === 'job')
            return 'occupation';
        if (norm === 'incomerange' || norm === 'income')
            return 'incomeRange';
        if (norm === 'riskcategory' || norm === 'risk')
            return 'riskCategory';
        if (norm === 'segment' || norm === 'customersegment')
            return 'segment';
        if (norm === 'status' || norm === 'customerstatus')
            return 'status';
        if (norm === 'branchcode' || norm === 'branch')
            return 'branchCode';
        if (norm === 'accountnumber' || norm === 'accnum')
            return 'accountNumber';
        if (norm === 'accounttype' || norm === 'acctype')
            return 'accountType';
        if (norm === 'balance' || norm === 'accbalance')
            return 'balance';
        if (norm === 'preferredcontact' || norm === 'contactpref')
            return 'preferredContact';
        if (norm === 'preferredlanguage' || norm === 'language')
