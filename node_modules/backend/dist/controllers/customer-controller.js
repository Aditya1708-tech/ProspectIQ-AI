...
export class CustomerController {
    ...
    async import(req, res) {
        ...
        try {
            const summary = await this.importService.importFromBuffer(file.buffer, file.mimetype, columnMapping, user.id);
            return res.status(200).json({
                success: true,
                data: summary,
                meta: {
    ...
    async getProfileAndAnalyze(req, res) {
        const user = req.user;
        const { id } = req.params;
        if (!user) {
            res.status(401).json({
    ...
    async relationshipHealth(req, res) {
        try {
            const analysis = await this.getProfileAndAnalyze(req, res);
            if (!analysis)
                return;
    ...
    async predictionTimeline(req, res) {
        try {
            const analysis = await this.getProfileAndAnalyze(req, res);
            if (!analysis)
                return;
    ...
    async getSimulationTemplates(req, res) {
        try {
            const templates = [
                {
