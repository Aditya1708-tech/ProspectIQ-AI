...
import { UserContext, RoleCode, AuthResponse } from 'shared';
...
export class AuthService {
  ...
  private generateAccessToken(user: { id: string; username: string; name: string; roles: { role: { code: string } }[] }): string {
    const roles: RoleCode[] = user.roles.map(ur => ur.role.code as RoleCode);
  ...
  async login(username: string, passwordPlain: string): Promise<AuthResponse> {
    ...
    // Log audit event
    const rolesList: RoleCode[] = user.roles.map(ur => ur.role.code as RoleCode);
  ...
  async getCurrentUser(userId: string): Promise<UserContext> {
